...
__host__ __device__ void max_heapfy(int *vec, int *index_vec, int n,int i){
...
__host__ __device__ void build_max_heap(int *vec, int *index_vec, int n){
    int i, last_parent = parent(n-1);
    for(i=last_parent;i>=0;i--) max_heapfy(vec,index_vec,n,i);
   
}

__device__ int *k_smallest_with_heap(int *vec,int n, int _k){
    int *heap;
    heap = (int *)malloc(_k * sizeof(int));
    int *index_vec;
...
__global__ void compute_distances(int *d_data, int *d_query, int *d_distances_r, int *d_indexes,int _k, int step){
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    int tid = threadIdx.x;
    int block_adj = blockIdx.x * _k;
    
    __shared__ int thread_distances[T];
    ...
    if(i < M){
        ...
        int dist = l1_distance(d_query,vec,N);
...
__host__ int *simultaneous_k_smallest(int *vec, int *indexes, int n, int _k){
    int *heap;
    heap = (int *)malloc(_k * sizeof(int));
    int *index_vec;
    index_vec = (int *)malloc(_k * sizeof(int));
...
int main(){
    ...
    int distances[k*numBlocks*steps];
    ...
    for(int step = 0; step < steps; step++){
        ...
        cudaMalloc((void**)&d_distances_r, k*numBlocks*sizeof(int));
        ...
        cudaMemcpy(&(distances[step*k*numBlocks]),d_distances_r,k*numBlocks*sizeof(int),cudaMemcpyDeviceToHost);
