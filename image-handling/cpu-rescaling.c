    ...
    for(row = 1; row < height-1; row = row + 3){
        for(col = 1; col < width-1; col = col + 3){
            for(i = 0; i < num_components; i++){
                pix = image_data[row*num_components*width + col*num_components + i];
                output_image[row*num_components*width + col*num_components + i] = pix;
                output_image[(row-1)*num_components*width + col*num_components + i] = pix;
                output_image[(row+1)*num_components*width + col*num_components + i] = pix;
                output_image[row*num_components*width + (col-1)*num_components + i] = pix;
                output_image[row*num_components*width + (col+1)*num_components + i] = pix;
                output_image[(row-1)*num_components*width + (col-1)*num_components + i] = pix;
                output_image[(row-1)*num_components*width + (col+1)*num_components + i] = pix;
                output_image[(row+1)*num_components*width + (col+1)*num_components + i] = pix;
                output_image[(row+1)*num_components*width + (col-1)*num_components + i] = pix;
